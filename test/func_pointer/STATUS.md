...
- ✅ = currently verifies (`make -C test/func_pointer`).
...
translates and verifies cleanly. Validate with `make -C test/func_pointer`.
...
| `assign_from_agg` | 3 | ✅ | Read a func-ptr out of a stack array into a local, then call it. Same recipe as `use_array_slot` (below): the emitter let-binds the stored value before `array_write` (so `array_spec_initd` stays provable at the read), then `valid_cast` the read-back value onto the `of_fn` validity before the call. |
...
| `array_runtime_idx` | 5 | ✅ | Runtime-indexed fnptr array `tbl[i]` with `_requires(i==0||i==1)`. Same recipe as `use_array_slot` (both slots `= add`, read `tbl[i]` into `f`, `of_fn_valid`, `valid_cast _ $(f)`, call, `drop_is_valid`). Because both slots hold `add` and `i∈{0,1}`, the runtime-index read is provably `add`, so `array_spec_initd`/`array_spec_mask` at `i` and the `valid_cast` value equality both discharge. |
...
- **Stage 3 — Join family:** `reassign_join_call`, `reassign_join` **✅ verify**
  ...
  local, then called, via the `use_array_slot` recipe (emitter let-binds the write;
  `valid_cast`).
- **Stage 4 — Fnptr as data:** `use_struct_field`, `union_field`, `malloc_fp`,
  ...
  **✅ verify**: after the stack-local fixed-array read fix (`emit.rs ExprT::Index`
  ...
  higher-order term blocks the `array_spec_initd` SMT E-match (fixed by the emitter
  let-binding the stored value before `array_write`), and the array-read value is
  only *provably* equal to the stored `of_fn` (fixed by a new `valid_cast` library
  ghost that re-keys the `is_valid` fact across a provable pointer equality). See
  the table rows.
