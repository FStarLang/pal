...
ghost fn uint8_t_agree (a: ptr) (#p1 #p2: perm) (#x #y: U8.t)
  ...
  ensures   pure (x == y)
{
  unfold uint8_t_pts_to a p1 x;
  unfold uint8_t_pts_to a p2 y;
  mem_pts_to_injective a;
  uint8_t_repr_injective x y (encode (SZ.v uint8_t_sizeof) None (U8.v x));
  fold uint8_t_pts_to a p1 x;
  fold uint8_t_pts_to a p2 y;
}
  ...
  ensures  uint8_t_pts_to a (p /. 2.0R) x ** uint8_t_pts_to a (p /. 2.0R) x
{
  unfold uint8_t_pts_to a p x;
  mem_share a;
  fold uint8_t_pts_to a (p /. 2.0R) x;
  fold uint8_t_pts_to a (p /. 2.0R) x;
}
...
{
  unfold uint8_t_pts_to a 1.0R x;
