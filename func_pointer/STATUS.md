...
| `assign_from_agg` | 3 | ❌ | Deferred: array-element lvalue read (Stage-4 machinery). |
...
| `use_array_slot` | 4 | ❌ | **Deferred (emitter gap).** The store `tbl[0] = add` (`array_write`) is fine, but reading a stack-local fixed-array element of func-ptr type lowers to `array_spec_idx (!var_tbl) i`, which is ill-typed: `array_spec_idx` needs a pure `array_spec` (Seq), `!var_tbl` is a runtime `array`. `src/pass/emit.rs` (`ExprT::Index`) sets `use_spec_idx = is_fixed_array` for any fixed-array rvalue — correct only for pure spec values (globals / by-value struct fields), not for stack arrays, which need `array_read`. No annotation changes the read lowering. |
| `multilayer` | 4 | ❌ | **Deferred (same emitter gap).** Array-elem → struct-field → call. The struct half works (as in `use_struct_field`), but the value source `tbl[0]` is the same stack-local fixed-array read that lowers to the ill-typed `array_spec_idx (!var_tbl) 0`. Needs the same `emit.rs ExprT::Index` fix. |
...
| `array_runtime_idx` | 5 | ❌ | Advanced (not yet attempted): runtime-indexed fnptr array (same stack-array `array_spec_idx` read gap as `use_array_slot`). |
...
- **Stage 3 — Join family:** `reassign_join_call`, `reassign_join` **✅ verify**
  ...
  `assign_from_agg` remains deferred (array-element read — the same Stage-4
  `array_spec_idx` gap).
- **Stage 4 — Fnptr as data:** `use_struct_field`, `union_field`, `malloc_fp`,
  ...
  spec composes — see the table row). `use_array_slot` and `multilayer` remain
  **deferred** on the stack-local fixed-array element read (`array_spec_idx
  (!var_tbl) i` is ill-typed; needs an `emit.rs ExprT::Index` fix to emit
  `array_read`). See the table rows for details.
- **Stage 5 — Advanced / edge:** `designated_vtable`, `dispatch`/`use_dispatch`,
  ...
  **`_refine_value` field contract** (`ops_c_valid this vo = pts_to this vo **
  ...
  `array_runtime_idx` remains
  **not attempted** (stack-array element read gap), and
