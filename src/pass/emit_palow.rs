...
const HEADER: &str = r#"(* Generated by pal --palow.
   ...
   Inline Pulse is not translated yet, so a contract that used it is dropped
   and a body that used it is `admit()`ed. Both make the specification weaker
   than the one PAL emits today rather than wrong; the count of them is the
